...
export const TypingRow = memo(forwardRef<TypingRowHandle, Props>(
  function TypingRow({ onRowComplete, onProgress, onAllDone }, ref) {
      ...
      typeField(row.code, 'fast', () => {
        ...
        typeField(row.trade, 'fast', () => {
          ...
          typeField(row.content, 'slow', () => {
            ...
            // Notify progress: this row is done, queue may be empty
            onProgress?.({
              displayed: completedCountRef.current,
              queued: queueRef.current.length,
              currentChar: 0,
            ...
            timerRef.current = setTimeout(() => {
              // Add to displayedRows AFTER completing animation to avoid duplicate
              onRowComplete(row)
              setCurrentRow(null)
              setTypedText('')
              setStatus('idle')
