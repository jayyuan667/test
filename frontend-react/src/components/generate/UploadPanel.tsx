...
export function UploadPanel({ onFileSelected, disabled, previewUrls, onFullscreen, annotationShapes, imageNaturalSize, labelColors }: Props) {
    ...
    if (dropzoneRef.current) {
      if (reduceMotion) {
        gsap.set(dropzoneRef.current, {
          ...
          borderColor: dragOver ? '#f97316' : '#cbd5e1',
        })
        return
      }
      if (dragOver) {
  ...
  }, [pageIdx, previewUrls, reduceMotion])
      ...
      <div className="flex flex-col h-full min-h-0">
        ...
        <div className="flex items-center justify-between gap-3 px-5 pt-4 pb-3 shrink-0">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button className="btn btn-ghost !px-2.5 !py-1.5 text-[11px] min-h-[44px]" onClick={() => setZoom(1)}>重置</button>
            {onFullscreen && (
              <button className="btn btn-ghost !px-2.5 !py-1.5 text-[11px] min-h-[44px]" aria-label="全屏预览" onClick={onFullscreen}>
  ...
  return (
    <div className="flex flex-col h-full min-h-0 p-5">
      <div
        ref={dropzoneRef}
        className={`dropzone flex-1 cursor-pointer ${dragOver ? 'drag-over' : ''} ${disabled ? 'opacity-40 pointer-events-none' : ''}`}
