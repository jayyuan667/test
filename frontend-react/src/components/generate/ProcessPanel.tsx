...
import { normalizeProcessRows, displayStepCode } from '../../utils/processRows'
...
export function ProcessPanel({ result, taskId, runToken, streamingChunks, reviewText, onTypewriterComplete, onRowsChange, onTypewriterProgress, onCommitSuccess }: Props) {
  ...
  const processStatusText = finalRows.length > 0
    ? `${finalRows.length} 道工序`
    : displayedRows.length > 0
      ? `工艺生成中... ${displayedRows.length} 道工序`
      : isStreaming
        ? '工艺生成中...'
        : '等待生成...'
  ...
  const rows = hasFinalResult
    ? allRows
    : allRows.filter(r => (r.content || '').trim().length > 0 || (r.trade || '').trim().length > 0)
