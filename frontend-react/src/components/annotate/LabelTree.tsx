...
export function LabelTree({ shapes, labels, selectedId, onSelect, onDelete, onRename }: Props) {
  ...
  // Auto-scroll sidebar to selected item when selectedId changes (from canvas click)
  useEffect(() => {
      ...
      {Array.from(grouped.entries()).map(([label, items]) => (
        <div key={label}>
          ...
          {!collapsedGroups.has(label) && items.map((shape, idx) => (
            <div
              key={shape.id}
              ref={el => { if (el) itemRefs.current.set(shape.id, el); else itemRefs.current.delete(shape.id) }}
              className={`
                flex items-center gap-2 pl-7 pr-2 py-1 rounded-lg cursor-pointer transition-all duration-100
                ${selectedId === shape.id ? 'bg-flame-50/80 ring-1 ring-flame-300' : 'hover:bg-slate-50/60'}
              `}
