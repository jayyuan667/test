...
import { normalizeAssetUrls, normalizeProcessRows, normalizeStringArray, displayStepCode } from '../utils/processRows'
...
function getScopeUiLabel(scope: Pick<LibraryScope, 'scope_type' | 'library_name'> | { scope_type: string; library_name?: string } | null | undefined) {
  if (!scope) return '工艺库'
  if (scope.scope_type === 'public') return '平台工艺库'
  return scope.library_name || '个人工艺库'
}
...
export function DbPage({ onNavigate }: { onNavigate: (id: PageId) => void }) {
  ...
  const renderPreviewThumb = (record: LibraryRecord) => {
    ...
    return (
      ...
      >
        <div className="w-full h-full flex flex-col items-center justify-center gap-1 text-orange-400 bg-orange-50/50 transition-transform duration-200 group-hover/thumb:scale-105">
          ...
          <span className="text-[10px] font-medium">快照</span>
        </div>
      </button>
    )
  }
    ...
    <div className="flex flex-col gap-3 h-full min-h-0">
      ...
      <div ref={toolbarRef} className="flex items-center justify-between shrink-0 flex-wrap gap-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1.5">
            <label htmlFor="db-page-size" className="text-[11px] font-semibold" style={{ color: 'var(--text-secondary)' }}>页大小</label>
            <select
              id="db-page-size"
              className="theme-form-field rounded-lg px-2.5 py-1.5 text-[12px] transition-all"
              value={pageSize}
