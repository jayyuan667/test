...
type WorkflowStage = 'idle' | 'analysis' | 'annotation' | 'feature' | 'review' | 'process' | 'completed'
...
function mapBackendStatusToStage(backendStatus: string): WorkflowStage {
  switch (backendStatus) {
    case 'awaiting_annotation': return 'annotation'
...
export function GeneratePage({ onError, onSuccess, onBusyChange }: { onError: (msg: string) => void; onSuccess?: (msg: string) => void; onBusyChange?: (busy: boolean) => void }) {
  ...
  const connectStream = useCallback((tid: string) => {
    ...
    esRef.current = connectSSE(tid, {
      onStepStart(data) {
        const stepName = String(data.name || data.step_name || '')
        const message = String(data.message || stepName || '处理中...')
        ...
        } else if (stepName.includes('特征提取') || message.includes('特征提取') || message.includes('缓存命中')) {
          if (alreadyGenerating) return
          if (STAGE_ORDER[workflowStageRef.current] < STAGE_ORDER['feature']) {
            workflowStageRef.current = 'feature'
          }
      ...
      async onAnnotationRequired(data) {
        if (data.task_id && activeTaskIdRef.current && data.task_id !== activeTaskIdRef.current) return
        if (processSubmissionLockedRef.current) return
        if (STAGE_ORDER[workflowStageRef.current] >= STAGE_ORDER['feature']) return
  ...
  const [retrievalName, setRetrievalName] = useState('默认知识库')
  const [featureCache, setFeatureCache] = useState(true)
  ...
  // Status reconciliation: poll backend state as a fallback when SSE events are
  // missed. Only runs before the 'process' stage — the existing completion
  // polling effect covers 'process' → 'completed'.
  useEffect(() => {
    if (!taskId) return
    if (STAGE_ORDER[workflowStageRef.current] >= STAGE_ORDER['process']) return
    if (processSubmissionLockedRef.current) return

    let cancelled = false
    const timer = window.setInterval(async () => {
      if (cancelled) return
      if (activeTaskIdRef.current !== taskId) { cancelled = true; return }

      try {
        const res = await getStatus(taskId)
        if (cancelled || !res) return

        const backendStage = mapBackendStatusToStage(res.status)
        const currentStage = workflowStageRef.current

        // Only move forward, never backward
        if (STAGE_ORDER[backendStage] <= STAGE_ORDER[currentStage]) return

        if (backendStage === 'annotation' && STAGE_ORDER[currentStage] < STAGE_ORDER['annotation']) {
          workflowStageRef.current = 'annotation'
          setStatus('awaiting_annotation')
          holdYoloReviewProgress()
          setPhaseHint('等待人工补全标注')
        ...
        if (backendStage === 'review' && STAGE_ORDER[currentStage] < STAGE_ORDER['review']) {
          // Fetch review data only if not already complete
          const reviewReady = reviewText && previewUrls.length > 0
          if (!reviewReady) {
            const result = await getResult(taskId)
            const r = result as Record<string, unknown>
            if (r?.review_text) setReviewText(r.review_text as string)
            const urls = (r?.preview_image_urls as string[]) || []
            if (urls.length) setPreviewUrls(urls)
          }
          clearProgressPlan()
          workflowStageRef.current = 'review'
          setStatus('awaiting_review')
          setPhaseHint('请审阅特征报告')
          if (progress < PROGRESS.featureDone) setPlannedProgress(PROGRESS.featureDone)
        }

        if (res.status === 'completed' && STAGE_ORDER[currentStage] < STAGE_ORDER['completed']) {
          const result = await getResult(taskId)
          if (result) {
            setResult(result as unknown as TaskResult)
            const rawUrls = (result as Record<string, unknown>).preview_image_urls as string[] || []
            if (rawUrls.length) {
          ...
          workflowStageRef.current = 'completed'
          setStatus('completed')
          finishProcessProgress(800)
        }

        if (res.status === 'cancelled' && STAGE_ORDER[currentStage] < STAGE_ORDER['completed']) {
          workflowStageRef.current = 'completed'
          setStatus('error')
          setPhaseHint('任务已取消')
        }
      ...
      {showRetrievalModal && (
        <div className="fixed inset-0 z-[9000] flex items-center justify-center" onClick={() => setShowRetrievalModal(false)}>
          ...
          <div className="theme-surface-panel relative rounded-2xl shadow-2xl border max-w-[420px] w-[90vw] p-6" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-[12px] mb-4" style={{ color: 'var(--text-muted)' }}>工艺生成时用于 RAG 检索的知识库，默认使用后端配置的检索库。</p>
            <div className="mb-4">
