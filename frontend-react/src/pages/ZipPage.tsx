...
function resolveTargetLibrary(target: LibraryTarget, scopes: LibraryScope[]): ZipImportReport['target_library'] {
  const selected = target.key ? scopes.find(scope => scope.library_key === target.key) : undefined
  const libraryName = selected?.library_name || target.name || target.key || (target.mode === 'public' ? '平台基线库' : '未命名工艺库')
  return {
    library_key: target.key || selected?.library_key || '',
...
const ZIP_RUN_POLL_MS = 3000
const ZIP_ACTIVE_RUN_STORAGE_KEY = 'zip_active_import_run_id'

const WORKFLOW_STAGE_TITLES: Record<string, string> = {
  received: '接收 ZIP',
  permission_checked: '权限校验',
  cache_checked: '缓存校验',
  validating_cached_result: '缓存可信校验',
  validating: '结果校验',
  rolling_back: '自动回滚',
  publishing_cache: '发布缓存',
  completed: '完成',
}

function getWorkflowStageTitle(stage: string): string {
  return WORKFLOW_STAGE_TITLES[stage] || stage || '运行事件'
}

type WorkflowTone = 'neutral' | 'success' | 'warning' | 'danger'

function getWorkflowTone(level: string, stage: string, failed: boolean): WorkflowTone {
  if (failed && stage !== 'completed') return 'danger'
  if (level === 'warning' || stage === 'rolling_back') return 'warning'
  if (stage === 'completed') return 'success'
  return 'neutral'
}

function workflowToneClass(tone: WorkflowTone): string {
  if (tone === 'success') return 'zip-workflow-node-success'
  if (tone === 'warning') return 'zip-workflow-node-warning'
  if (tone === 'danger') return 'zip-workflow-node-danger'
  return 'zip-workflow-node-neutral'
}

function getWorkflowRunId(report: ZipImportReport | null, run: ZipImportRun | null): string {
  return report?.workflow_run_id || run?.run_id || ''
}

function isZipImportBusy(report: ZipImportReport | null, run: ZipImportRun | null): boolean {
  return report?.error_code === 'zip_import_busy' || run?.error_code === 'zip_import_busy'
}

function getWorkflowStatusTitle(report: ZipImportReport | null, run: ZipImportRun | null): string {
  if (report?.rollback?.status === 'rollback_failed' || run?.error_code === 'rollback_failed') return '自动回滚异常'
  if (isZipImportBusy(report, run)) return '入库等待'
  if (run?.status === 'failed' || report?.validation?.status === 'failed') return '入库失败'
  if (report?.cached) return '复用历史入库结果'
  return '入库完成'
}

function getWorkflowStatusDescription(report: ZipImportReport | null, run: ZipImportRun | null): string {
  if (report?.rollback?.status === 'rollback_failed' || run?.error_code === 'rollback_failed') {
    return '入库校验失败且自动回滚异常，部分数据可能需要管理员处理。请复制运行编号并检查目标工艺库数据完整性。'
  }
  if (report?.rollback?.status === 'completed') return '入库校验失败，本次写入已撤销。'
  if (isZipImportBusy(report, run)) return run?.error_message || report?.message || '当前已有用户正在入库，请稍等。'
  if (report?.cached) return '复用历史入库结果，已重新校验可访问性。'
  if (report?.validation?.status === 'passed') return '入库完成，结果校验已通过。'
  if (run?.error_message) return run.error_message
  return report?.message || '本次入库已返回结果。'
}
...
function isTerminalZipRunStatus(status: string): boolean {
  return status === 'completed' || status === 'failed'
}

async function pollZipImportRun(
  runId: string,
  shouldContinue: () => boolean,
  onRun: (run: ZipImportRun) => void,
  onPollError?: (err: unknown, attempt: number) => void,
): Promise<ZipImportRun | null> {
  let errorCount = 0
  while (shouldContinue()) {
    try {
      const run = await getZipImportRun(runId)
      if (!shouldContinue()) return null
      errorCount = 0
      onRun(run)
      if (isTerminalZipRunStatus(run.status)) return run
    } catch (err) {
      if (!shouldContinue()) return null
      errorCount += 1
      onPollError?.(err, errorCount)
    }
    await wait(ZIP_RUN_POLL_MS)
  }
  return null
}

function buildZipReportFromPartial(
  partial: Partial<ZipImportReport> | undefined,
  fallback: {
    zipName: string
  ...
  const summary = source.summary || {
    ...
    imported_count: 0,
    skipped_count: 0,
    error_count: fallback.errorCode ? 1 : 0,
  }
  return {
    batch_id: source.batch_id || '',
    zip_name: source.zip_name || fallback.zipName,
    conflict_mode: source.conflict_mode || fallback.conflictMode,
    library_mode: source.library_mode || fallback.target.mode,
    cached: source.cached,
    message: source.message || fallback.message,
    workflow_run_id: source.workflow_run_id || fallback.workflowRunId,
    validation: source.validation,
    rollback: source.rollback,
    error_code: source.error_code || fallback.errorCode,
    summary,
    matched_pairs: source.matched_pairs || [],
    unmatched_pdfs: source.unmatched_pdfs || [],
    unmatched_xlsx: source.unmatched_xlsx || [],
    unmatched_prts: source.unmatched_prts || [],
    unmatched_images: source.unmatched_images || [],
    errors: source.errors || (fallback.errorCode ? [{ error: fallback.message || fallback.errorCode }] : []),
    target_library: source.target_library || resolveTargetLibrary(fallback.target, fallback.scopes),
...
async function countZipEntries(file: File): Promise<number | null> {
  ...
  for (let offset = bytes.length - eocdMinSize; offset >= 0; offset -= 1) {
    if (
      ...
      if (totalEntries <= 0 || totalEntries >= 0xffff) return null
      if (centralDirectorySize <= 0 || centralDirectoryOffset < 0 || centralDirectoryOffset >= file.size) return null

      const centralDirectory = await file.slice(
        centralDirectoryOffset,
        centralDirectoryOffset + centralDirectorySize,
      ).arrayBuffer()
      const cdBytes = new Uint8Array(centralDirectory)
      const cdView = new DataView(centralDirectory)
      const decoder = new TextDecoder('utf-8')
      let cdOffset = 0
      let realFiles = 0

      for (let entryIndex = 0; entryIndex < totalEntries && cdOffset + 46 <= cdBytes.length; entryIndex += 1) {
        if (
          cdBytes[cdOffset] !== 0x50 ||
          cdBytes[cdOffset + 1] !== 0x4b ||
          cdBytes[cdOffset + 2] !== 0x01 ||
          cdBytes[cdOffset + 3] !== 0x02
        ) {
          return null
        }

        const fileNameLength = cdView.getUint16(cdOffset + 28, true)
        const extraLength = cdView.getUint16(cdOffset + 30, true)
        const commentLength = cdView.getUint16(cdOffset + 32, true)
        const externalAttrs = cdView.getUint32(cdOffset + 38, true)
        const nameStart = cdOffset + 46
        const nameEnd = nameStart + fileNameLength
        if (nameEnd > cdBytes.length) return null

        const rawName = decoder.decode(cdBytes.slice(nameStart, nameEnd)).replace(/\\/g, '/')
        const parts = rawName.split('/').filter(Boolean)
        const baseName = parts[parts.length - 1] || ''
        const isDirectory = rawName.endsWith('/') || ((externalAttrs >>> 16) & 0x4000) !== 0
        const isMacMetadata = (
          parts.includes('__MACOSX') ||
          baseName === '.DS_Store' ||
          baseName.startsWith('._')
        )
        if (!isDirectory && !isMacMetadata && baseName) realFiles += 1

        cdOffset = nameEnd + extraLength + commentLength
      }

      return realFiles > 0 ? realFiles : null
    }
  }
...
function WorkflowRunSummary({
  ...
  report: ZipImportReport | null
  run: ZipImportRun | null
  copied: boolean
  onCopy: () => void
}) {
  const runId = getWorkflowRunId(report, run)
  const title = getWorkflowStatusTitle(report, run)
  const description = getWorkflowStatusDescription(report, run)
  const isDanger = report?.rollback?.status === 'rollback_failed' || run?.error_code === 'rollback_failed'

  return (
    <section
      className={`zip-workflow-summary rounded-2xl border p-4 ${isDanger ? 'zip-workflow-summary-danger' : ''}`}
      aria-live="polite"
    ...
    >
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          ...
          <div className="text-[12px] mt-1 leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{description}</div>
        </div>
        {runId && (
          <button
            type="button"
            className="btn btn-secondary !text-[12px] !py-2 shrink-0"
            onClick={onCopy}
            aria-label={`复制运行编号 ${runId}`}
          >
            {copied ? '已复制' : '复制运行编号'}
          </button>
        )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-2 mt-4">
        <div className="zip-workflow-kv">
          <span>运行编号</span>
          <strong>{runId || '-'}</strong>
        </div>
        <div className="zip-workflow-kv">
          <span>批次号</span>
          <strong>{report?.batch_id || run?.batch_id || '-'}</strong>
        </div>
        <div className="zip-workflow-kv">
          <span>目标库</span>
          <strong>{report?.target_library?.library_name || run?.library_key || '-'}</strong>
        </div>
      </div>
    </section>
  )
...
function WorkflowTimeline({
  ...
  return (
    <section className="theme-surface-panel rounded-2xl border p-4" style={{ borderColor: 'var(--border)' }}>
      <div className="text-[14px] font-bold mb-3" style={{ color: 'var(--text-primary)' }}>运行轨迹</div>
      {error && (
      ...
      {!error && !run && (
        <div className="text-[12px]" style={{ color: 'var(--text-secondary)' }}>正在读取运行轨迹...</div>
      )}
      {!error && run && events.length === 0 && (
        <div className="text-[12px]" style={{ color: 'var(--text-secondary)' }}>已记录运行，但暂无事件明细。</div>
      )}
      {!error && events.length > 0 && (
        <ol className="zip-workflow-timeline">
          {events.map((event, index) => {
            const tone = getWorkflowTone(event.level, event.stage, failed && index === events.length - 1)
            return (
              <li key={`${event.stage}-${index}`} className={`zip-workflow-node ${workflowToneClass(tone)}`}>
                <div className="zip-workflow-node-dot" aria-hidden="true" />
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="text-[13px] font-semibold" style={{ color: 'var(--text-primary)' }}>
                      {getWorkflowStageTitle(event.stage)}
                    </span>
                    {event.progress_hint != null && (
                      <span className="theme-surface-chip rounded-full border px-2 py-0.5 text-[10px] font-mono">
                        {event.progress_hint}%
                      </span>
                    )}
                  </div>
...
function WorkflowValidationSummary({ report }: { report: ZipImportReport }) {
  ...
  return (
    <section className="theme-surface-panel rounded-2xl border p-4" style={{ borderColor: 'var(--border)' }}>
      <div className="text-[14px] font-bold mb-2" style={{ color: 'var(--text-primary)' }}>校验结果</div>
      <div className="text-[12px] mb-3 leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{text}</div>
      <div className="grid grid-cols-3 gap-2">
        <div className="zip-workflow-metric"><span>期望</span><strong>{validation?.expected_count ?? '-'}</strong></div>
        <div className="zip-workflow-metric"><span>写入</span><strong>{validation?.inserted_count ?? '-'}</strong></div>
        <div className="zip-workflow-metric"><span>可见</span><strong>{validation?.visible_count ?? '-'}</strong></div>
      </div>
      {validation?.error_code && (
        <div className="mt-3 text-[11px] font-mono" style={{ color: 'var(--text-secondary)' }}>{validation.error_code}</div>
      )}
    </section>
  )
}

function WorkflowCacheSummary({ report, run }: { report: ZipImportReport; run: ZipImportRun | null }) {
  const hasCacheEvent = (run?.events || []).some(event => event.stage === 'cache_checked' || event.stage === 'validating_cached_result')
  if (!report.cached && !hasCacheEvent) return null

  return (
    <section className="theme-surface-panel rounded-2xl border p-4" style={{ borderColor: 'var(--border)' }}>
      <div className="text-[14px] font-bold mb-2" style={{ color: 'var(--text-primary)' }}>
        {report.cached ? '缓存已复用' : '缓存校验'}
      </div>
      <div className="text-[12px] leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
        {report.cached
          ? '复用历史入库结果，已重新校验可访问性。'
...
function WorkflowRollbackSummary({ report }: { report: ZipImportReport }) {
  const rollback = report.rollback
  if (!rollback) return null

  const failed = rollback.status === 'rollback_failed'
  const text = failed
    ? '自动回滚异常，可能需要管理员处理。'
  ...
  return (
    <section
      className={`theme-surface-panel rounded-2xl border p-4 ${failed ? 'zip-workflow-summary-danger' : ''}`}
      style={{ borderColor: failed ? 'rgba(220, 38, 38, 0.35)' : 'var(--border)' }}
      role={failed ? 'alert' : undefined}
    >
      <div className="text-[14px] font-bold mb-2" style={{ color: 'var(--text-primary)' }}>回滚结果</div>
      <div className="text-[12px] leading-relaxed" style={{ color: 'var(--text-secondary)' }}>{text}</div>
      <div className="grid grid-cols-2 gap-2 mt-3">
        <div className="zip-workflow-metric"><span>删除</span><strong>{rollback.deleted_records}</strong></div>
        <div className="zip-workflow-metric"><span>恢复</span><strong>{rollback.restored_snapshots ?? 0}</strong></div>
      </div>
      {rollback.error && (
        <div className="mt-3 text-[11px] font-mono break-words" style={{ color: 'var(--text-secondary)' }}>{rollback.error}</div>
      )}
      {failed && (
        <div className="mt-3 text-[12px] leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
          处理建议：复制运行编号，检查目标库和批次记录，并按 workflow_run_id 定位本次写入和回滚快照。
        </div>
      )}
    </section>
  )
...
export function ZipPage({ onBusyChange }: { onBusyChange?: (busy: boolean) => void }) {
  ...
  const [newLibNameError, setNewLibNameError] = useState('')
  ...
  const [workflowRun, setWorkflowRun] = useState<ZipImportRun | null>(null)
  ...
  const defaultScopeInitializedRef = useRef(false)
  ...
  const setProgressValue = useCallback((next: number, options?: { allowDecrease?: boolean }) => {
    const clamped = Math.max(0, Math.min(100, next))
    if (!options?.allowDecrease && clamped < progressPercentRef.current) return
  ...
  const startEstimatedProgress = useCallback((fileCount: number, runId: number) => {
    ...
    setProgressHint(`预计 ${safeFileCount} 个有效文件，按每个约 2 分钟估算，进度每 3 秒平滑推进。`)
  ...
  const loadWorkflowRun = useCallback(async (runId: string) => {
    setWorkflowRunError('')
    try {
      const data = await getZipImportRun(runId)
      setWorkflowRun(data)
    } catch (err) {
      const message = err instanceof Error ? err.message : '运行轨迹暂不可用'
      setWorkflowRunError(message || '运行轨迹暂不可用')
      setWorkflowRun(null)
    }
  }, [])
  ...
  useEffect(() => {
    if (defaultScopeInitializedRef.current) return
    if (user?.role !== 'super_admin') return
    if (!scopes.some(scope => scope.library_key === 'public')) return
    defaultScopeInitializedRef.current = true
  ...
  useEffect(() => {
    if (!scopes.length || uploadRunIdRef.current !== 0) return
    const activeRunId = sessionStorage.getItem(ZIP_ACTIVE_RUN_STORAGE_KEY)
    if (!activeRunId || restoredRunIdRef.current === activeRunId) return

    restoredRunIdRef.current = activeRunId
    let cancelled = false
    const restoreRunId = uploadRunIdRef.current + 1
    uploadRunIdRef.current = restoreRunId
    const fallbackTarget = currentTarget()
    const fallbackScopes = visibleScopes

    clearProgressTimers()
    setBusy(true)
    setStatusZone('running')
    setPhaseText('正在恢复后台入库状态...')
    setProgressHint(`运行编号：${activeRunId}`)
    setProgressValue(ZIP_PROGRESS_INITIAL, { allowDecrease: true })
    setWorkflowRunError('')

    void (async () => {
      try {
        const finalRun = await pollZipImportRun(activeRunId, () => !cancelled && uploadRunIdRef.current === restoreRunId, run => {
          setWorkflowRun(run)
          setWorkflowRunError('')
          const lastEvent = run.events?.[run.events.length - 1]
          if (lastEvent?.message) setPhaseText(lastEvent.message)
          if (run.progress_hint != null) {
            setProgressValue(Math.min(run.status === 'completed' ? 100 : ZIP_PROGRESS_CAP, run.progress_hint))
          }
        }, (err, attempt) => {
          const message = err instanceof Error ? err.message : '状态查询暂时失败'
          setWorkflowRunError(`${message}，正在第 ${attempt} 次重试。`)
          setPhaseText('状态查询暂时失败，后台入库仍在继续...')
          setProgressHint(`运行编号：${activeRunId}，稍后自动重试。`)
        })
        if (cancelled || uploadRunIdRef.current !== restoreRunId || !finalRun) return

        const result = buildZipReportFromPartial(finalRun.result, {
          zipName: finalRun.result?.zip_name || '后台入库任务',
          conflictMode,
          target: fallbackTarget,
          scopes: fallbackScopes,
          workflowRunId: finalRun.run_id,
          message: finalRun.error_message,
          errorCode: finalRun.error_code,
        })
        setReport(result)

        if (finalRun.status === 'failed') {
          setStatusZone('error')
          setPhaseText(getWorkflowStatusDescription(result, finalRun))
          setProgressHint(finalRun.run_id ? `运行编号：${finalRun.run_id}` : '')
          return
        }

        setProgressValue(100)
        setPhaseText('入库完成')
        setProgressHint(result.workflow_run_id ? `运行编号：${result.workflow_run_id}` : '')
        setStatusZone('done')
        sessionStorage.setItem('zip_unlocked', 'true')
        loadScopes()
      } catch (err) {
        if (cancelled || uploadRunIdRef.current !== restoreRunId) return
        const message = err instanceof Error ? err.message : '状态查询失败'
        setWorkflowRunError(message || '状态查询失败')
        setStatusZone('error')
        setPhaseText(`状态查询失败：${message}`)
        setProgressHint(activeRunId ? `运行编号：${activeRunId}` : '')
      } finally {
        if (!cancelled && uploadRunIdRef.current === restoreRunId) {
          setBusy(false)
        }
      }
    })()
  ...
  }, [scopes.length, clearProgressTimers, loadScopes, setProgressValue])

  const handleUpload = useCallback(async (file: File) => {
    if (!file.name.toLowerCase().endsWith('.zip')) return

    // Require a meaningful name for new personal libraries
    if (selectedScope === '__new__' && !newLibName.trim()) {
      setNewLibNameError('请输入工艺库名称。')
      return
    }
    setNewLibNameError('')
    ...
    try {
      ...
      const accepted = await importZipZipAsync({
        file,
      ...
      if (uploadRunIdRef.current !== runId) return
      clearProgressTimers()
      sessionStorage.setItem(ZIP_ACTIVE_RUN_STORAGE_KEY, accepted.workflow_run_id)
      setWorkflowRunError('')
      setProgressHint(`运行编号：${accepted.workflow_run_id}`)

      const finalRun = await pollZipImportRun(accepted.workflow_run_id, () => uploadRunIdRef.current === runId, run => {
        setWorkflowRun(run)
        setWorkflowRunError('')
        const lastEvent = run.events?.[run.events.length - 1]
        if (lastEvent?.message) setPhaseText(lastEvent.message)
        if (run.progress_hint != null) {
          setProgressValue(Math.max(progressPercentRef.current, Math.min(run.status === 'completed' ? 100 : ZIP_PROGRESS_CAP, run.progress_hint)))
        }
      }, (err, attempt) => {
        const message = err instanceof Error ? err.message : '状态查询暂时失败'
        setWorkflowRunError(`${message}，正在第 ${attempt} 次重试。`)
        setPhaseText('状态查询暂时失败，后台入库仍在继续...')
        setProgressHint(`运行编号：${accepted.workflow_run_id}，稍后自动重试。`)
      })
      if (uploadRunIdRef.current !== runId || !finalRun) return
      ...
      if (result.cached) {
        setPhaseText('复用历史入库结果，正在校验可访问性...')
      ...
      setProgressHint(
        result.validation?.status === 'passed'
          ? (result.workflow_run_id ? `运行编号：${result.workflow_run_id}` : '')
          : ''
      )
      setStatusZone('done')
      sessionStorage.setItem('zip_unlocked', 'true')
      ...
      if (err instanceof ApiError && err.body && typeof err.body === 'object') {
        const body = err.body as Partial<ZipImportReport> & { error?: string; error_code?: string }
        const failedReport: ZipImportReport = {
          batch_id: body.batch_id || '',
          zip_name: body.zip_name || file.name,
          conflict_mode: body.conflict_mode || conflictMode,
          library_mode: body.library_mode || target.mode,
          cached: body.cached,
          ...
          unmatched_pdfs: body.unmatched_pdfs || [],
          unmatched_xlsx: body.unmatched_xlsx || [],
          unmatched_prts: body.unmatched_prts || [],
          unmatched_images: body.unmatched_images || [],
          errors: body.errors || [{ error: body.error || err.message }],
          target_library: body.target_library || resolveTargetLibrary(target, visibleScopes),
          created_at: body.created_at || new Date().toISOString(),
        }
        setReport(failedReport)
        setWorkflowRun(null)
        setWorkflowRunError('')
        if (failedReport.workflow_run_id) {
          void loadWorkflowRun(failedReport.workflow_run_id)
        }
        setStatusZone('error')
        setPhaseText(getWorkflowStatusDescription(failedReport, null))
        setProgressHint('')
        return
      }
      ...
      const rollbackHint = message.includes('撤销') || message.includes('回滚')
        ? message
        : `错误：${message}`
      setPhaseText(rollbackHint)
      setProgressHint('')
    } finally {
      if (uploadRunIdRef.current === runId) {
        setBusy(false)
  ...
  }, [animateProgressTo, clearProgressTimers, currentTarget, conflictMode, loadScopes, loadWorkflowRun, newLibName, selectedScope, setProgressValue, startEstimatedProgress, visibleScopes])
  ...
  const handleCopyRunId = useCallback(() => {
    const runId = getWorkflowRunId(report, workflowRun)
    if (!runId) return
  ...
  const handleReset = useCallback(() => {
    ...
    clearProgressTimers()
    sessionStorage.removeItem(ZIP_ACTIVE_RUN_STORAGE_KEY)
    setStatusZone('idle')
    setReport(null)
    setWorkflowRun(null)
    setWorkflowRunError('')
    ...
    setProgressHint('')
    setProgressValue(0, { allowDecrease: true })
    setBusy(false)
    setMatchedPage(1)
    setUnmatchedPage(1)
    setNewLibNameError('')
  ...
  const unmatchedPdfs = report?.unmatched_pdfs || []
  const unmatchedOtherFiles = [
    ...(report?.unmatched_prts || []),
    ...(report?.unmatched_xlsx || []),
  ...
  const unmatchedTotalPages = Math.max(1, Math.max(
    Math.ceil(unmatchedOtherFiles.length / UNMATCHED_PAGE_SIZE),
  ...
  const unmatchedOtherItems = unmatchedOtherFiles.slice((unmatchedPage - 1) * UNMATCHED_PAGE_SIZE, unmatchedPage * UNMATCHED_PAGE_SIZE)
  const unmatchedPdfItems = unmatchedPdfs.slice((unmatchedPage - 1) * UNMATCHED_PAGE_SIZE, unmatchedPage * UNMATCHED_PAGE_SIZE)
  const isZipBusyResult = statusZone === 'error' && isZipImportBusy(report, workflowRun)

  // Log lines — prefer backend events when available
  const eventLogLines = workflowRun?.events?.map(event => ({
    ts: event.created_at,
    line: `${getWorkflowStageTitle(event.stage)}：${event.message || event.stage}`,
  })) || []

  const fallbackLogLines = report ? [
    { ts: report.created_at, line: `接收到 ZIP 批次 ${report.zip_name || '工艺包.zip'}。` },
    ...(report.workflow_run_id ? [{ ts: report.created_at, line: `运行编号 ${report.workflow_run_id}。` }] : []),
    { ts: report.created_at, line: `图纸 ${report.summary?.prt_count || 0} 个，PDF ${report.summary?.pdf_count || 0} 个。` },
    { ts: report.created_at, line: `已匹配 ${report.summary?.matched_pairs || 0} 组，导入 ${report.summary?.imported_count || 0} 条。` },
    ...(report.validation?.status === 'passed'
      ? [{ ts: report.created_at, line: `校验通过：可见 ${report.validation.visible_count ?? 0} 条，写入 ${report.validation.inserted_count ?? 0} 条。` }]
      : []),
    { ts: report.created_at, line: `其他未匹配文件 ${unmatchedOtherFiles.length} 项，未匹配 PDF ${unmatchedPdfs.length} 项。` },
    ...errors.slice(0, 3).map(err => ({
      ts: report.created_at,
      line: `错误：${err.prefix || err.pdf_name || err.image_name || err.prt_name || '批次项'} - ${err.error || err.message || '解析失败'}`,
    })),
  ] : []

  const logLines = eventLogLines.length ? eventLogLines : fallbackLogLines
  ...
  const heroDescription = user?.role === 'super_admin'
    ? '默认写入平台基线库；如需隔离验证，可手动选择已有个人库或新建项目库。'
    : user?.role === 'enterprise_admin'
      ? '默认推荐先写入个人工艺库，先确认本次导入结果；本阶段企业范围仅做界面引导，不扩展后端入库范围。'
      : '默认推荐先写入个人工艺库，确认后再决定共享。这样最安全，也方便后续只看自己的入库结果。'
  const helperText = user?.role === 'enterprise_admin'
    ? '本阶段企业范围仅做界面引导；真实入库仍写入你当前可用的个人工艺库。'
    : user?.role === 'super_admin'
      ? '超管默认写入平台基线库；新建私有库需要先选择明确企业上下文。'
    : '默认推荐先写入个人工艺库，确认后再决定共享。'
              ...
              >
                {visibleScopes.map(s => (
                  <option key={s.library_key} value={s.library_key}>
                    {s.library_name}（{s.scope_type === 'public' ? '平台基线库' : '个人工艺库'}{s.record_count != null ? ` · ${s.record_count}条` : ''}）
                  </option>
                ))}
            ...
            {selectedScope === '__new__' && (
              <div className="flex flex-col gap-2 mb-3">
                <input
                  ...
                  value={newLibName}
                  onChange={e => { setNewLibName(e.target.value); setNewLibNameError('') }}
                  disabled={busy}
                />
                {newLibNameError && (
                  <div className="text-[12px]" style={{ color: 'var(--danger)' }}>{newLibNameError}</div>
                )}
      ...
      <div ref={workspaceRef} className="card-solid flex flex-col gap-4">
          ...
          <div ref={statusRef} className="rounded-2xl border p-5 theme-surface-panel" style={{ borderColor: 'var(--surface-empty-accent-border)', background: 'linear-gradient(135deg, rgba(255,253,249,0.98) 0%, rgba(255,243,232,0.82) 100%)' }}>
            ...
            {report && (
              <div className="flex gap-5 text-[12px]" style={{ color: 'var(--text-secondary)' }}>
                ...
                <span>未匹配 <strong style={{ color: 'var(--text-primary)' }}>{unmatchedOtherFiles.length + unmatchedPdfs.length}</strong></span>
        ...
        {(statusZone === 'done' || statusZone === 'error') && (
          <div
            ...
            `}
          >
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${isZipBusyResult ? 'bg-amber-100' : statusZone === 'error' ? 'bg-red-100' : 'bg-emerald-100'}`}>
              <span className={`text-[24px] ${isZipBusyResult ? 'text-amber-600' : statusZone === 'error' ? 'text-red-500' : 'text-emerald-500'}`}>
                {isZipBusyResult ? '!' : statusZone === 'error' ? '✕' : '✓'}
              </span>
            </div>
            <div className="flex-1 min-w-0">
              <div className={`text-[15px] font-bold ${isZipBusyResult ? 'text-amber-700' : statusZone === 'error' ? 'text-red-700' : 'text-emerald-700'}`}>
                {statusZone === 'error' ? getWorkflowStatusTitle(report, workflowRun) : report?.cached ? '复用历史入库结果' : '入库完成'}
              </div>
              <div className={`text-[12px] mt-0.5 ${isZipBusyResult ? 'text-amber-600' : statusZone === 'error' ? 'text-red-500' : 'text-emerald-600'}`}>
                {statusZone === 'error'
                  ? phaseText
                  : report?.cached
        ...
        {report && (
          <div ref={resultRef} className="flex flex-col gap-3">
            <WorkflowRunSummary
              ...
              run={workflowRun}
              copied={copiedRunId}
              onCopy={handleCopyRunId}
            />

            <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1.25fr)_minmax(320px,0.75fr)] gap-3">
              <WorkflowTimeline run={workflowRun} error={workflowRunError} />
              <div className="flex flex-col gap-3">
                <WorkflowValidationSummary report={report} />
                <WorkflowCacheSummary report={report} run={workflowRun} />
                <WorkflowRollbackSummary report={report} />
              </div>
            </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex gap-2 flex-wrap">
                            ...
                            <span className="chip">目标库：{report.target_library?.library_name || report.target_library?.library_key || '未命名工艺库'}</span>
                          </div>
                        </div>
                      )
                    })}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <div className="theme-surface-panel-muted rounded-xl border p-3" style={{ borderColor: 'var(--border)' }}>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {unmatchedOtherItems.length ? unmatchedOtherItems.map((f, i) => (
            ...
            {resultTab === 'log' && (
              <div className="rounded-xl bg-gradient-to-br from-slate-900 to-slate-800 text-slate-200 p-4 font-mono text-[12px] leading-relaxed max-h-[240px] overflow-auto">
                {logLines.map((entry, i) => {
                  ...
                  return (
                    <div key={`${entry.ts}-${i}`} className="py-1 px-2 rounded hover:bg-white/5 transition-colors">
