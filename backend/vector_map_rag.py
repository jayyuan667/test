...
try:
    from .config import DB_DATA_FOLDER, VECTOR_DB_PATH
except ImportError:
    try:
        from backend.config import DB_DATA_FOLDER, VECTOR_DB_PATH
    except ImportError:
        DB_DATA_FOLDER = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "db_data")
        VECTOR_DB_PATH = os.path.join(DB_DATA_FOLDER, "2d-v.db")

# Use configurable runtime paths so deployments can keep data outside code.
BASE_DIR = os.getenv("FEATURIZER_BASE_DIR") or os.path.dirname(DB_DATA_FOLDER)
DB_PATH = os.getenv("VECTOR_DB_PATH") or (
    os.path.join(os.getenv("FEATURIZER_BASE_DIR"), "db_data", "2d-v.db")
    if os.getenv("FEATURIZER_BASE_DIR")
    else VECTOR_DB_PATH
)
