...
from .config import TASK_STORE_DB_FILE

DB_FILE = TASK_STORE_DB_FILE
...
def clear_task_events(task_id: str) -> None:
    """Delete all SSE events for a task from the DB.

    Used when a task is re-run so _db_event_generator doesn't replay stale events."""
    with _conn() as c:
