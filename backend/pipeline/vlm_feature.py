...
from ..config import OUTPUT_FOLDER, create_openai_client
...
【其他特征】（描述槽、凸台、台阶、布置关系、吊面/背面需翻面加工的特征等语义信息；如无填"无"）
...
【外形尺寸】
格式（按零件截面形状选择其一）：
  - 圆形截面（轴类/盘类/法兰/套筒）→ Ø直径×总厚 或 Ø直径×总长，如 Ø63.17×16.4
  - 非圆形截面（板类/块类/支架）→ 长×宽×厚，如 282×129×21.5
判断逻辑：
  - 先看主视图：若长宽尺寸相同且形状为圆形（标注含Ø符号），按圆形截面格式输出
  - 再综合侧视图/俯视图确认
值的选择：
  - 直径取最大外圆标注值（带Ø符号的值）
  - 厚度/总长取整体通长尺寸，不取螺纹长度/台阶高/局部槽深
  - 若同一方向有多个标注值（外轮廓272 vs 内腔246），取最大值
  - 台阶/法兰/凸缘的外延属于外形轮廓一部分，不得忽略
  - 无明确整体标注时填"无"；只抄图纸数字，不估算
  - **⚠ 高度/厚度取整体轮廓总高（重要）：**
    - 若有唯一整体尺寸标注，直接使用该值
    - 若无整体标注，但侧视图中标注了多段叠加尺寸（如主体高15.3 + 底座高2.2），
      则总高 = 各段之和（如 15.3+2.2=17.5），**叠加已标注分段是"读取"，不是"估算"，允许执行**
    - 错误示例：侧视图标注了15.3和2.2两段，却只填10.2（单段局部尺寸）❌
    - 正确示例：侧视图标注了15.3和2.2，总高填17.5 ✅
【通孔】数量-直径，多种孔径用分号分隔
【沉孔沉槽】沉孔写直径×角度或直径×深度，沉槽写宽×深
【螺纹孔】数量-规格 孔深N 底孔深N，平底或通孔需注明
【特殊孔】钢丝螺套、销孔、铆孔、穿丝孔（线切割穿丝用的小孔，通常φ1~φ5，成对出现）等特殊孔型
【尺寸公差】逐张扫描以下位置：
...
【刻字】字高×深度×内容，逐处列出
【吊面/翻面特征】零件是否存在需翻面加工的背面特征：
  检查依据（逐项核对）：
  ① 图纸中是否有"仰视图"——仰视图的存在直接意味着背面有需加工的特征
  ② 仰视图中是否有孔（通孔/螺纹孔/沉孔）、槽、凸台等加工特征
  ③ 剖视图中是否有开口朝下的腔槽（从正面无法直接加工）
  ④ 技术要求中是否有"翻面"、"背面加工"、"反面"等字样
  ⑤ 主视图/侧视图底部外轮廓线上是否有粗糙度符号（Ra/Rz）——底面有精加工要求时需吊面
  ⑥ 主视图/侧视图底部是否有虚线（隐藏线）表示底面孔或槽——无仰视图时的底面特征
  ⑦ 技术要求中是否有"吊孔"、"起吊"、"吊面"、"吊装"等字样
  ⑧ 图纸中是否有专用起吊孔标注（孔旁注"吊装用"或带起重符号▲）
  ⑨ 底面是否有形位公差框格引线（平面度/平行度公差指向底面）
  输出规则：
  - 存在以上任一情形 → 写"存在，[具体描述，如：仰视图中4×M3螺纹孔深8；背面沉槽φ10深3]"
  - 完全不存在 → 填"无"
【零件名称】根据外形推断，如无法判断填"无"
【形态】一句话描述整体几何形态
【类型】功能类别
...
规则：
- 数字必须原样抄录，不得估算
- **⚠ 小数精度：标注值中的小数点及其后数字必须原样抄录，不得舍入、省略或合并为整数**
  - 错误示例：φ1.8 写成 φ18（漏小数点，差10倍）
  - 错误示例：φ3.8 写成 φ38
...
def _merge_geometry_creo_vlm_fields(
    ...
    lines = [
        ...
        f"【吊面/翻面特征】{_safe(drawing_fields.get('吊面/翻面特征', '无'))}",
        f"【其他特征】{other_val}",
    ]
