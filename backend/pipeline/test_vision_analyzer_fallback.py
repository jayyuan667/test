...
import backend.pipeline.vision_analyzer as vision_analyzer
...
def test_encode_image_resizes_remote_payload_under_pixel_limit(monkeypatch, tmp_path):
    img = tmp_path / "large.png"
    Image.new("RGB", (120, 80), "white").save(img)
    analyzer = _make_analyzer()

    monkeypatch.setenv("VISION_SHARPEN", "0")
    monkeypatch.setattr(vision_analyzer, "VISION_MAX_TOTAL_PIXELS", 3_600)

    encoded = analyzer._encode_image(str(img))
    decoded = base64.b64decode(encoded)
    with Image.open(io.BytesIO(decoded)) as resized:
        assert resized.width * resized.height <= 3_600
