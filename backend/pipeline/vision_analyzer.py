...
VISION_MAX_TOTAL_PIXELS = int(os.getenv("VISION_MAX_TOTAL_PIXELS", "36000000"))
...
class VisionAnalyzer:
    ...
    def _prepare_remote_image(self, raw_bytes: bytes) -> Image.Image:
        img = Image.open(io.BytesIO(raw_bytes))
        img.load()
        total_pixels = img.width * img.height
        if total_pixels > VISION_MAX_TOTAL_PIXELS:
            scale = (VISION_MAX_TOTAL_PIXELS / total_pixels) ** 0.5
            next_size = (max(1, int(img.width * scale)), max(1, int(img.height * scale)))
            print(
                f"[Vision] 图像像素 {img.width}x{img.height}={total_pixels} 超过限制 "
                f"{VISION_MAX_TOTAL_PIXELS}，已缩放到 {next_size[0]}x{next_size[1]} 用于远程视觉分析"
            )
            img = img.resize(next_size, Image.Resampling.LANCZOS)
        return img

    def _encode_image(self, image_path: str) -> str:
        """读取、按远程视觉限制缩放并锐化图片，返回 base64 字符串。"""
        with open(image_path, "rb") as f:
            raw_bytes = f.read()
        img = self._prepare_remote_image(raw_bytes)
