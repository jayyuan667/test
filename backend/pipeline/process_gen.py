...
class ProcessGenerator:
    ...
    def _parse_markdown_process(self, raw_text: str) -> List[List[str]]:
        ...
        for content in self._coalesce_process_lines(raw_text):
            match = re.match(r"^(\d{4})\s*[:：@\-\|,，;；\s]*\s*([\s\S]+)$", content)
            if match:
                tag = match.group(1)
                proc_block = match.group(2).strip()
                if proc_block:
                    proc_lines = [line.strip() for line in proc_block.splitlines() if line.strip()]
                    proc_content = proc_lines[0] if proc_lines else proc_block
                    detail_content = "\n".join(proc_lines[1:]).strip()
                    trade_type = ""
                    # ── 新格式：工序内容 （工种：料）──
    ...
    def _emit_process_stream_lines(self, process_raw: str, stream_callback=None, log_callback=None, delay: float = 0.15):
    ...
    def generate(
        ...
        # ── MOCK: returns predefined valve stem process for specific image ──
        mock_prefix = os.getenv("MOCK_PREFIX", "").strip()
        if mock_prefix:
            prefix_raw = str(prefix_hint or "").strip().lower()
            stem = prefix_raw
            if stem == mock_prefix.lower():
                return _MOCK_VALVE_STEM_PROCESS(stream_callback, log_callback)
...
"""
    ...
    def _post_check_process(self, process_raw: str, constraints: dict) -> str:
        ...
        # ── ⑤ 独立翻面行工种规范化 ──
        def _fix_standalone_flip_trade(match):
            prefix = match.group(1)
            body = match.group(2)
            return f"{prefix}{body} （工种：翻面）"

        process_raw = re.sub(
            r'^((?:-\s*)?\d{4}\s*[:：]\s*)(翻面[，,、；;。\s][^\n]*?)\s*[（(]工种[：:][^）)]*[）)]\s*$',
            _fix_standalone_flip_trade,
            process_raw,
            flags=re.MULTILINE,
        )
...
- 独立翻面工序的工种必须写【翻面】，不要写数铣；真正执行背面铣削/钻攻的后续加工工序再按实际设备写数铣、钻等

**翻面工序格式模板（根据当前零件背面实际特征替换方括号内容，禁止照抄）：**
写法A（翻面独立工序，背面特征较多时推荐）：
  - 00XX: 以底面定位，铣削正面[特征描述]；钻攻正面[孔规格，如4×M6深10] （工种：数铣）
  - 00XX: 翻面，以正面为基准重新装夹，压紧底面两侧 （工种：翻面）
  - 00XX: 铣削背面[平面/槽，注明尺寸和粗糙度]；钻攻背面[孔规格] （工种：数铣）
写法B（翻面作工步，背面特征简单时可合并在同一工序）：
  - 00XX: 工步1：以底面定位，铣削正面[特征]；钻攻正面[孔规格]；工步2：翻面，以正面为基准重新装夹；铣削/钻攻背面[背面具体特征] （工种：数铣）
"""
...
# =============================================================================
# MOCK: predefined valve stem process for demo (test.png uploads)
# Activate:   add "MOCK_PREFIX=test" to .env
# Deactivate: remove that line
# =============================================================================

_MOCK_VALVE_STEM_ROWS = [
    ["0010","来料","来料为大锻件，已由锻造厂完成各表面的荒加工，锻件已按DZ2.6.60 技术条件生产、检查、验收，并附有锻件承制厂的合格证。"],
    ["0020","检验","1  锻件到厂后，核对质保书的完整性、正确性，按出厂合格证检查试样完整 性、锻件名称、图号、材料牌号、订货合同号炉号及出厂钢印标记是否完整； 2  按阀杆（交货图）D660T-274000A002F复查毛坯外形尺寸及表面粗糙度。"],
    ["0030","镗削","在一端面打中心孔B4。"],
    ["0040","车削","按图见光外圆及端面尺寸，表面粗糙度达Ra6.3；按阀杆（交货图） D660T-274000A002F在试样区切取试样、在图示位置打材料印记。"],
    ["0050","检验","力学性能试验、化学成分分析。"],
    ["0060","检验","外圆超声波探伤、本体硬度试验。"],
    ["0070","车削","各外圆单面留0.2～0.3mm余量车削，表面粗糙度达Ra6.3，总长尺寸暂不加工。"],
    ["0080","划线","在工艺头端(小端）距端面30mm位置划Φ16吊装孔线。"],
    ["0090","钻削","钻φ16吊装孔。"],
    ["0100","钳工","去毛刺，清理干净。"],
    ["0110","热处理","去应力退火。"],
    ["0120","车削","1  在大端打中心孔B4，修另一端中心孔；除总长尺寸不加工，φ126(-0.4,-0.425) 单面留0.06～0.08mm余量，螺纹外圆暂不加工外，其余各尺寸按图车准，保证形位公差要求； 2  上中心架，按φ126(-0.4,-0.425)找正在0.02mm以内，车准右端φ110外圆尺寸，右端球面先不加工，并按图打印材质。"],
    ["0130","划线","划A向视图花键线。"],
    ["0140","铣削","按A向视图铣准花键。"],
    ["0150","钳工","1  修花键上各处圆角； 2  去毛刺，清理干净。"],
    ["0160","磨削","1  修研中心孔； 2  按技术要求2检查弯曲度，并做好记录，检查配合并通知车间技术组处理弯曲情况； 3  按外圆找正，φ126(-0.4,-0.425)磨至φ126(-0.415,-0.425),保证形位公差要求。"],
    ["0170","检验","按图检查阀杆的弯曲情况并通知车间技术组处理。"],
    ["0180","热处理","按技术要求3渗氮。"],
    ["0190","车削","1  修研中心孔； 2  按技术要求2检查弯曲度，并做好记录，检查配合并通知车间技术组处理弯曲情况； 3  按外圆找正，抛光Φ126(-0.4,-0.425)各外圆，保证形位公差及粗糙度要求。"],
    ["0200","检验","按图检查阀杆的弯曲情况并通知车间技术组处理。"],
    ["0210","车削","1  修光外圆上各处圆角； 2  去左端工艺头，车准左端螺纹和端面，保证形位公差及粗糙度要求； 3  车准右端球面并抛光，保证总长尺寸、粗糙度要求。"],
    ["0220","划线","划A向视图上腰形凸台线。"],
    ["0230","镗削","按A向视图铣准腰形凸台。注意相对位置关系。"],
    ["0240","检验","按技术要求6进行着色检查。"],
    ["0250","钳工","去毛刺，清理干净；涂油。"],
    ["0260","入库","入库"],
]

def _MOCK_VALVE_STEM_PROCESS(stream_callback=None, log_callback=None):
    """Return the predefined valve stem process for demo."""
    if log_callback:
        log_callback("MOCK: returning predefined valve stem process (26 steps)")
    import time
    lines = []
    for code, trade, content in _MOCK_VALVE_STEM_ROWS:
        line = "{} @{} @{}".format(code, trade, content)
        if trade:
            line += " （工种：{}）".format(trade)
        lines.append(line)
    proc_raw = "\n".join(lines)
    if stream_callback:
        for line in lines:
            ...
            time.sleep(0.03)
    rows = [[c, t, ct] for c, t, ct in _MOCK_VALVE_STEM_ROWS]
    return proc_raw, rows, None
