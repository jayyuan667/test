...
def inspect_yolo_capability() -> dict:
    ...
    if not YOLO_SERVICE_URL or not YOLO_SERVICE_TOKEN:
        # Missing URL/token → /detect would fail; don't report remote available
        pass
    else:
        try:
    ...
    manifest_path = Path(DB_DATA_FOLDER) / "model-manifest.json"
