...
def test_pdf_upload_is_rejected_before_task_creation(monkeypatch):
    ...
    auth_fd, auth_path = tempfile.mkstemp(suffix=".db"); task_fd, task_path = tempfile.mkstemp(suffix=".db")
    os.close(auth_fd); os.close(task_fd)
    monkeypatch.setattr(auth_store, "DB_FILE", auth_path)
    monkeypatch.setattr(task_store, "DB_FILE", task_path)
    before_ids = set(upload_api.tasks)
    try:
        auth_store.init_auth_db(); task_store.init_db()
        ...
        assert set(upload_api.tasks) == before_ids
        assert task_store.list_tasks() == []
    finally:
        for path in (auth_path, task_path):
