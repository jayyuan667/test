...
def _is_stale_timestamp(value: str | None, stale_after_seconds: int) -> bool:
    ...
    if not ts:
        return False
    return (datetime.now() - ts).total_seconds() > stale_after_seconds
...
def mark_stale_workflow_failed(
    run_id: str,
    *,
    stale_after_seconds: int | None = None,
    error_code: str = "stale_worker_interrupted",
    error_message: str = "后台任务超时未更新，已释放入库并发锁。",
) -> bool:
    """Mark one running workflow as failed when its latest event is stale."""
    ensure_workflow_tables()
    ...
    try:
        run = conn.execute(
            "SELECT run_id, status, started_at FROM workflow_runs WHERE run_id = ?",
            (run_id,),
        ).fetchone()
        ...
        last_event = conn.execute(
            """
            SELECT created_at, progress_hint
        ...
        last_activity_at = last_event["created_at"] if last_event else run["started_at"]
        if not _is_stale_timestamp(last_activity_at, stale_seconds):
            return False

        now = _now()
        payload_json = json.dumps(
            {"error_code": error_code, "last_activity_at": last_activity_at},
            ensure_ascii=False,
        )
        conn.execute(
            """
        ...
        conn.execute(
            ...
            (
                run_id,
                error_message,
                last_event["progress_hint"] if last_event else None,
                payload_json,
                now,
            ),
        )
