...
_VECTOR_TABLE_COLUMNS = (
    ...
    ("source_file_hashes", "TEXT DEFAULT '[]'"),
...
def _fetch_existing_record(prefix: str, library_key: str = ""):
    ...
    return {
        ...
        "source_file_hashes": _parse_json_list(source_file_hashes),
...
def _update_record(record_id: int, payload: dict, library_key: str = ""):
    ...
    if "source_file_hashes" in payload:
        source_file_hashes = _json_list_text(payload.get("source_file_hashes"))
    else:
        source_file_hashes = _json_list_text(existing.get("source_file_hashes"))
    ...
    try:
        cursor.execute(
            ...
            SET prefix = ?, product_type = ?, process_summary = ?, context = ?, tech_requirement = ?, content = ?,
                source_type = ?, source_task_id = ?, preview_task_id = ?, preview_total_pages = ?, preview_image_urls = ?,
                source_file_hashes = ?, feature_report_text = ?, feature_report_path = ?
            WHERE id = ?
            """,
            (
                prefix,
...
def _upsert_record(draft: dict, replace: bool, library_key: str = ""):
    ...
    source_file_hashes = _json_list_text(draft.get("source_file_hashes"))
