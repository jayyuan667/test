...
def _duration_text_to_minutes(value):
    ...
    match = re.match(r"^(\d+(?:\.\d+)?)\s*(?:min|分钟)$", text)
    if match:
...
def _operation_event_from_enriched_row(task_id, row, index, status="streaming"):
    ...
    return {
        ...
        "trade": trade,
        "content": content,
        "equipment": [equipment_text] if equipment_text else [],
        "duration_minutes": _duration_text_to_minutes(duration_text),
        "parameters": [],
        "note": None,
        "status": status,
    }
...
def upload_drawing():
    ...
    def process_drawing(task_id, filepath):
        ...
        try:
            ...
            # ── Feature cache: use entry from early check ────────────────
            if _cache_entry:
