...
def assert_task_access(task_id):
    ...
    if task is None:
        try:
            ...
            logging.warning("[assert_task_access] SQLite lookup failed: task=%s", task_id, exc_info=True)
    ...
    if task is None:
        try:
            ...
            logging.warning("[assert_task_access] file lookup failed: task=%s", task_id, exc_info=True)
