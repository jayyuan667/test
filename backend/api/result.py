...
def get_result(task_id):
    ...
    if task_id in tasks:
        ...
        # awaiting_review: return feature report data stored in task dict
        if task.get("status") == "awaiting_review":
            return jsonify(_enrich_result_payload(task_id, {
                "task_id": task_id,
                "status": "awaiting_review",
                "progress": task.get("progress", 50),
                "feature_report": task.get("feature_report_text", task.get("review_text", "")),
                "feature_report_text": task.get("feature_report_text", ""),
                "feature_report_json": task.get("feature_report_json", {}),
                "review_text": task.get("review_text", ""),
                "raw_review_text": task.get("raw_review_text", ""),
                "vision_descriptions": task.get("vision_descriptions", []),
                "pdf_name": task.get("pdf_name", task.get("prt_name", "")),
                "preview_image_urls": task.get("preview_image_urls", []),
            }, task))
        # awaiting_annotation: return preview images
        if task.get("status") == "awaiting_annotation":
            return jsonify(_enrich_result_payload(task_id, {
                "task_id": task_id,
                "status": "awaiting_annotation",
                "progress": task.get("progress", 35),
                "pdf_name": task.get("pdf_name", task.get("prt_name", "")),
            }, task))
