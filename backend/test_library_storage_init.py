...
def test_ensure_scope_returns_enterprise_id(monkeypatch, tmp_path):
    db_path = _use_temp_database(monkeypatch, tmp_path)
    library_scope.initialize_library_storage()

    scope = library_scope.ensure_scope("private_demo", "Private Demo", enterprise_id=6)

    assert scope["enterprise_id"] == 6
    with sqlite3.connect(db_path) as conn:
        stored = conn.execute(
            "SELECT enterprise_id FROM kb_library_scopes WHERE library_key = ?",
            ("private_demo",),
        ).fetchone()[0]
    assert stored == 6
...
def test_commit_persists_retrieval_check(monkeypatch, tmp_path):
    _use_temp_database(monkeypatch, tmp_path)
    library_scope.initialize_library_storage()
    library_scope.ensure_scope("private_demo", "Private Demo", enterprise_id=1)
...
def test_regular_user_cannot_commit_to_public_library(monkeypatch, tmp_path):
    _use_temp_database(monkeypatch, tmp_path)
    library_scope.initialize_library_storage()
    ...
    with app.test_request_context(
        ...
        json={
            ...
            "draft": {
                "prefix": "D127",
                "feature_report_text": "【零件名称】D127",
                "process_list": [{"code": "0010", "trade": "车", "content": "车外圆"}],
            },
        },
    ):
        g.current_user = {"role": "user", "enterprise_id": 1}
        response, status = library_api.commit_library_record.__wrapped__()

    assert status == 403
    assert "Public library" in response.get_json()["error"]


def test_regular_user_cannot_clear_public_scope(monkeypatch, tmp_path):
    _use_temp_database(monkeypatch, tmp_path)
    library_scope.initialize_library_storage()

    app = Flask(__name__)
    with app.test_request_context("/api/library/scopes/public", method="DELETE"):
        g.current_user = {"role": "user", "enterprise_id": 1}
        response, status = library_api.clear_library_scope.__wrapped__("public")

    assert status == 403
    assert "Public library" in response.get_json()["error"]


def test_kb_preview_asset_requires_record_enterprise_access(monkeypatch, tmp_path):
    db_path = _use_temp_database(monkeypatch, tmp_path)
    monkeypatch.setattr(result_api, "KB_PREVIEW_FOLDER", str(tmp_path / "previews"))
    library_scope.initialize_library_storage()
    scope = library_scope.ensure_scope("private_b", "Private B", enterprise_id=2)

    preview_dir = tmp_path / "previews" / "kb_private_b"
    preview_dir.mkdir(parents=True)
    (preview_dir / "page.png").write_bytes(b"fake-png")
    with sqlite3.connect(db_path) as conn:
        conn.execute(
            f"INSERT INTO {scope['vector_table']} "
            "(prefix, content, real, enterprise_id, preview_task_id) VALUES (?, ?, ?, ?, ?)",
            ("B1", "[]", 1, 2, "kb_private_b"),
        )
        conn.commit()

    app = Flask(__name__)
    with app.test_request_context("/api/result/kb_private_b/asset/page.png"):
        g.current_user = {"role": "user", "enterprise_id": 1}
        response, status = result_api.get_result_asset.__wrapped__("kb_private_b", "page.png")

    assert status == 404

    with app.test_request_context("/api/result/kb_private_b/asset/page.png"):
        g.current_user = {"role": "user", "enterprise_id": 2}
        response = result_api.get_result_asset.__wrapped__("kb_private_b", "page.png")
