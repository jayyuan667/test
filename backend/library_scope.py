...
def ensure_vector_table(table_name: str):
    ...
    try:
        ...
        # Migration: add source_file_hashes column
        cursor.execute(f"PRAGMA table_info({table_name})")
        if "source_file_hashes" not in {r[1] for r in cursor.fetchall()}:
            cursor.execute(f"ALTER TABLE {table_name} ADD COLUMN source_file_hashes TEXT DEFAULT '[]'")
