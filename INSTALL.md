...
| 文件/目录 | 是否在 git 中 | 说明 |
|-----------|--------------|------|
| `db_data\2d-v.db` | 是，`git clone` 自动获取 | 若原机器有未提交的新数据，需手动 `git push` 后再 `clone` |
...
**Q: 知识库检索返回空**  
A: 确认 `db_data\2d-v.db` 存在（`git clone` 自动获取；若原机器有新数据需先 `git push`）。
