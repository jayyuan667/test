...
$w = 0
while ($w -lt 30) {
    $listening = Get-NetTCPConnection -LocalPort 5190 -State Listen -ErrorAction SilentlyContinue
    if ($listening) { break }
...
$w = 0
while ($w -lt 30) {
    $listening = Get-NetTCPConnection -LocalPort 3200 -State Listen -ErrorAction SilentlyContinue
    if ($listening) { break }
    Start-Sleep -Seconds 1
    $w++
}
...
Write-Host "  ============================================" -ForegroundColor Cyan
    ...
    Write-Host "    Backend:  http://localhost:5190/api/health" -ForegroundColor White
Write-Host "  ============================================" -ForegroundColor Cyan
Write-Host ""
Write-Host "  Streaming logs (Ctrl+C to stop all) ..." -ForegroundColor DarkGray
Write-Host "  --------------------------------------------" -ForegroundColor DarkGray

Start-Process "http://localhost:3200" -ErrorAction SilentlyContinue
